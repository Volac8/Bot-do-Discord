...
VISIBLE_COGS = ["agenda", "duelo", "humor", "ia", "personalizados"]
...
class HelpCommand(commands.HelpCommand):
    async def send_bot_help(self, mapping):
        ...
        embed.set_footer(text=f"Run {commands.Context.clean_prefix}{self.invoked_with} [cog] to learn more about a cog and its commands")
    ...
    async def send_group_help(self, group):
        ...
        embed.description = f"```yaml\n---\n{desc}\n---\n```"
        embed.set_footer(text=f"Run {commands.Context.clean_prefix}{self.invoked_with} [command] to learn more about a command")
        await self.get_destination().send(embed=embed)
